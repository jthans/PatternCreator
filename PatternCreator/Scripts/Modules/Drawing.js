		...
		cleanupPolygons() {
			...
			for (var i = 0; i < App.drawnPolygons.length; i++) {
				for (var j = 0; j < App.drawnPolygons.length; j++) {
					...
					if (App.drawnPolygons[j].isNested) {
						continue;
					}
	...
	class Polygon {
		...
		containsLine(p1, p2) {
			for (var i = 0; i < this.points.length; i++) {
				if (this.points[i] == p1 ||
					this.points[i] == p2) {
					var lastPoint = i - 1 < 0 ? this.points[this.points.length - 1] : this.points[i - 1];
		...
		containsPoint(pointInQuestion) {
			...
			for (var i = 0; i < this.points.length; i++) {
				...
				if (p1.yLoc == p2.yLoc ||
					(p1.yLoc == pointInQuestion.yLoc && p2.yLoc < pointInQuestion.yLoc) ||
					(p2.yLoc == pointInQuestion.yLoc && p1.yLoc < pointInQuestion.yLoc)) {
					numIntersections += 2;
					continue;
				} else if ((p1.yLoc == pointInQuestion.yLoc && p2.yLoc > pointInQuestion.yLoc) ||
					(p2.yLoc == pointInQuestion.yLoc && p1.yLoc > pointInQuestion.yLoc)) {
					numIntersections--;
					continue;
				}
